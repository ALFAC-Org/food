...
public class ListarPedidosPorStatusUseCaseTest {
    ...
    public void deveListarPedidosPorStatus(StatusPedido status) throws FoodException {
        ...
        // Act
        List<Pedido> execute = new ListarPedidosPorStatusUseCase(pedidoRepository).executar(status);

        // Assert
        assertThat(execute).isNotNull();
        assertThat(execute).isNotEmpty();
        assertThat(execute).hasSize(pedidos.size());
        assertThat(execute.get(0).getStatus()).isEqualTo(status);
    }
}